...
content.addEventListener("click", (e) => {
    if (e.target.className === "add-cart-btn") {
        ...
        addToLocalStorage("home", addCartButtonId, tShirtData, counterObj[Number(addCartButtonId)], countInCart);
    ...
    if (e.target.className === "remove-btn") {
        ...
        const addRemoveBtns = document.querySelectorAll(".add-remove-btns"); //why array is empty when keeping it on top
        removeFromLocalStorage("home", removeButtonId, tShirtData, counterObj[Number(removeButtonId)], addToCartBtn[Number(removeButtonId)], addRemoveBtns[Number(removeButtonId)], countInCart);
    }
    if (e.target.className === "add-btn") {
        const addButtonId = e.target.dataset.addbuttonid;
        addToLocalStorage("home", addButtonId, tShirtData, counterObj[Number(addButtonId)], null);
    }
});
